...
from . import BackboneMeta


class MobileNetMeta(BackboneMeta):
    allowed_backbones = ['mobilenet128', 'mobilenet160', 'mobilenet192', 'mobilenet224']

    def __init__(self, backbone):
        super(MobileNetMeta, self).__init__(backbone)
    ...
    def validate(self):
        ...
        if backbone not in MobileNetMeta.allowed_backbones:
            raise ValueError('Backbone (\'{}\') not in allowed backbones ({}).'.format(backbone, MobileNetMeta.allowed_backbones))
